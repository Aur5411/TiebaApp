...
interface ITiebaApi {
     ...
     * @param forumName 吧名
     * @param tbs tbs
     * @param fromWidget 是否模拟「手机桌面小组件」入口（1 = 是，可获得双倍签到经验）
     */
    fun signFlow(
        forumId: String,
        forumName: String,
    ...
    /**
     * 通过「贴吧 UID」（tieba_uid，个人主页 ID）反查用户信息。
     *
     * 注意：贴吧有「user_id」和「tieba_uid」两套不同的 ID。
     * 用户输入/分享文案里的 `#数字#` 是 tieba_uid，而 profile / userpost 等接口要的是 user_id，
     * 因此需要先通过本接口（cmd=309702）把 tieba_uid 转成 user_id。
     */
    fun getUserByTiebaUidFlow(
        tiebaUid: String,
    ): Flow<GetUserByTiebaUidResponse>
